export default {
  async fetch(request, env) {
    ...
    // Detalhada por preposto — base publicada pelo script diário (CMS_DETALHADA_GERAL)
    // e pela tela de admin do app. O resumo fica num arquivo à parte: assim o app
    ...
    // Converte texto -> base64 sem criar copias gigantes da string.
    // O caminho antigo, btoa(unescape(encodeURIComponent(s))), fazia TRES
    // copias do conteudo inteiro antes de codificar. Numa base de 3 MB isso
    // estourava o limite de memoria do worker: a Cloudflare matava a
    // requisicao com um erro DELA, que nao carrega os cabecalhos de CORS -
    // e o navegador reportava "Failed to fetch", escondendo a causa real.
    ...
    async function gitCreateBlob(owner, repo, contentStr) {
      const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/git/blobs`, {
        ...
        body: JSON.stringify({ content: textoParaBase64(contentStr), encoding: "base64" })
      });
    ...
    async function saveFile(url, content, sha, message, compacto = false) {
      const { owner, repo, path } = parseRepoUrl(url);
      const branch = env.GITHUB_BRANCH || "main";
      ...
      // compacto = sem indentacao. Só a detalhada usa: são 3 MB que a
      // indentacao de 2 espacos transformava em 9 MB antes do base64.
      // Os demais arquivos continuam legiveis no diff do GitHub.
      const jsonStr = compacto ? JSON.stringify(content) : JSON.stringify(content, null, 2);
    ...
    try {
      ...
      if (request.method === "PATCH") {
        ...
        // Base que o app dos vendedores lê. Publicada pelo script diário e
        // pela tela de admin — nenhum dos dois passa por commit manual, e por
        // isso não esperam o GitHub Pages reconstruir o site inteiro.

        // getDetalhadaInfo → só o resumo (alguns bytes). O app consulta isto a
        // cada abertura e só baixa a base inteira quando `gerado` mudou.
        if (body.action === "getDetalhadaInfo") {
          try {
            ...
            // Sem resumo o app apenas baixa a base; não é motivo de erro.
        ...
        // Fora de WRITE_ACTIONS pelo mesmo motivo de savePerformance e saveRfv:
        // quem publica é um script na máquina da CMS e uma tela de navegador, e
        // nenhum dos dois consegue guardar o ADMIN_TOKEN sem expô-lo. A proteção
        // aqui é a validação do payload — recusar lixo é o que importa, porque
        // uma base quebrada apagaria a de todos os vendedores e o erro só
        ...
        if (body.action === "saveDetalhada" && body.data && typeof body.data === "object") {
          ...
          await saveFile(GITHUB_DETALHADA, payload, sha,
            `detalhada: base por ${payload.por || "?"} (${base.l.length} linhas)`, true);

          // O resumo vai depois da base: se a base falhar, ninguém fica achando
          // que publicou. Resumo velho incomoda menos que base velha.
          const info = {
